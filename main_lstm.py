...
class LSTMModel(nn.Module):
    ...
    def forward(self, x):
        x = x.unsqueeze(1)  # Ensure input shape consistency
        out, _ = self.lstm(x)
        return self.fc(out.squeeze(1))  # Apply fully connected layer

class HeiderModel(LSTMModel):
...
def create_visualizations_with_decision_boundaries():
    ...
    # --- Plot 9: Decision Boundary for LSTM Model ---
    ax = axs[1, 2]
    plot_decision_boundary(models['LSTM'], X_train, y_train, "(G) Decision Boundary\n(LSTM)", ax)

    # --- Plot 10: Decision Boundary for Heider-LSTM Model ---
    ax = axs[1, 3]
    plot_decision_boundary(models['Heider-LSTM'], X_train, y_train, "(H) Decision Boundary\n(Heider-LSTM)", ax)

    plt.tight_layout()
    plt.savefig('result_2_lstm.jpg', dpi = 600)
    plt.show()
...
models = {
    ...
    'Heider-LSTM': HeiderModel(input_dim, hidden_dim, output_dim, lambda_reg=0.01)
}
