...
def create_visualizations_with_decision_boundaries():
    ...
    # --- Plot 9: Decision Boundary for GRU Model ---
    ax = axs[1, 2]
    plot_decision_boundary(models['GRU'], X_train, y_train, "(G) Decision Boundary\n(GRU)", ax)

    # --- Plot 10: Decision Boundary for Heider-GRU Model ---
    ax = axs[1, 3]
    plot_decision_boundary(models['Heider-GRU'], X_train, y_train, "(H) Decision Boundary\n(Heider-GRU)", ax)

    plt.tight_layout()
    plt.savefig('result_2_GRU.jpg', dpi = 600)
    plt.show()
...
models = {
    ...
    'Heider-GRU': HeiderModel(input_dim, hidden_dim, output_dim, lambda_reg=0.01)
}
